...
const RowContainer = ({ flag,data}) => {
  ...
  return (
        ...
        //<div className="w-full py-12">
            <Splide hasTrack={false} className="w-full py-12" options={{perPage:5, gap:10}}> 
                <SplideTrack>
                    {data && data.length>0 ? (data.map((item) =>(
                            <SplideSlide key={item.id} className="w-300 h-[250px] min-w-[300px] md:w-340 md:min-w-[340px] bg-cardOverlay rounded-lg p-2 my-12 backdrop-blur-lg hover:drop-shadow-lg flex flex-col items-center justify-between" >
